...
def saved_depth_into_uint16_image(data,path,depth_scale=256):
...
def saved_confidence_into_uint8_image(data,path,scale=256):
    data = data * scale
...
def do_scalecano_test_with_custom_data(
    ...
    for i in tqdm(range(0, len(test_data), bs)):
        ...
        # Process the batch
        pred_depths, confidence,outputs = get_prediction(
            model=model,
        ...
        current_batch_data = batch_data[0]
        input_rgb_folder = current_batch_data['folder']
        input_rgb_fname = current_batch_data['filename']
        

        pred_depths, confidence = postprocess_per_image_depth_and_conf(pred_depth=pred_depths.squeeze(0).squeeze(0), 
                                             pred_conf=confidence.squeeze(0).squeeze(0), 
                                             rgb_origin=rgb_origins[0], 
                                             pad=pads[0], 
                                             normalize_scale=normalize_scale, 
                                             scale_info=label_scale_factors[0])
        
        pred_depths = pred_depths.cpu().numpy()
        confidence = confidence.cpu().numpy()
        

        saved_cam_folder = input_rgb_folder.replace(cfg.dataset_root_path,cfg.show_dir)
        os.makedirs(saved_cam_folder,exist_ok=True)
        
        saved_est_dpt_m_name = os.path.join(saved_cam_folder,os.path.basename(input_rgb_fname)[:-4]+"_dpt.png")
        saved_est_conf_m_name = os.path.join(saved_cam_folder,os.path.basename(input_rgb_fname)[:-4]+"_conf.png")


        saved_depth_into_uint16_image(pred_depths,saved_est_dpt_m_name)
        saved_confidence_into_uint8_image(confidence,saved_est_conf_m_name)





def postprocess_per_image_depth_and_conf(pred_depth, pred_conf, rgb_origin, pad, normalize_scale, scale_info):
    ...
    pred_conf = pred_conf[pad[0] : pred_conf.shape[0] - pad[1], pad[2] : pred_conf.shape[1] - pad[3]]
    pred_conf = torch.nn.functional.interpolate(pred_conf[None, None, :, :], [rgb_origin.shape[0], rgb_origin.shape[1]], mode='bilinear').squeeze() # to original size

    return pred_depth,pred_conf
