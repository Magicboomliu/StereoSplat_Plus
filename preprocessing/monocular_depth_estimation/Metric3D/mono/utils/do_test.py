...
def do_scalecano_test_with_custom_data(
    ...
    for i in tqdm(range(0, len(test_data), bs)):
        ...
        # saved_location
        saved_cam_folder = input_rgb_folder.replace("sweeps","sweeps_dptm")
        os.makedirs(saved_cam_folder,exist_ok=True)
        saved_est_dpt_m_name = os.path.join(saved_cam_folder,os.path.basename(input_rgb_fname)[:-4]+"_dpt.npy")
        saved_est_conf_m_name = os.path.join(saved_cam_folder,os.path.basename(input_rgb_fname)[:-4]+"_conf.npy")

        np.save(saved_est_dpt_m_name,pred_depths)
        np.save(saved_est_conf_m_name,confidence)
