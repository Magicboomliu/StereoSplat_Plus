...
class CameraPerspective(Camera):
    ...
    def __init__(self, root_dir, seq='2013_05_28_drive_0009_sync', cam_id=0):
        ...
        pose_dir = os.path.join(root_dir, 'data_poses', seq) # data pose
        calib_dir = os.path.join(root_dir, 'calibration')   # calibiration
    ...
    def cam2image(self, points):
        ...
        points_proj = np.matmul(self.K[:3,:3].reshape([1,3,3]), points) #[1,3,N]
        depth = points_proj[:,2,:]
        depth[depth==0] = -1e-6
