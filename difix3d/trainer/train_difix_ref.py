...
def main(args):
    ...
    dataset_train = KITTI360_Restoration_Dataset(dataset_path=args.dataset_path, 
                                  split="train", 
    ...
    dataset_val = KITTI360_Restoration_Dataset(dataset_path=args.dataset_path, 
                                split="test", 
                                tokenizer=net_difix.tokenizer,
                                height=args.resolution_h, width=args.resolution_w,
                                use_relative_pose=args.use_relative_pose)
    ...
    for epoch in range(0, args.num_training_epochs):
        for step, batch in enumerate(dl_train):
            ...
            with accelerator.accumulate(*l_acc):
                ...
                L = restoration_losses(x_tgt_pred, x_tgt, args)
                loss = L["loss"]
                mse_loss = L["mse"]
                loss_l2 = L["loss_l2"]
                loss_ssim = L["loss_ssim"]
                ssim_value = L["ssim_metric"]
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process:
                    ...
                    logs["train/ssim"] = ssim_value.detach().item()
                    logs["train/psnr"] = psnr_neg1_to_1(mse_loss.detach().item())
                    ...
                    # compute validation set MSE / PSNR / SSIM
                    if args.eval_freq > 0 and global_step % args.eval_freq == 1:
                        ...
                        for step, batch_val in enumerate(dl_val):
                            ...
                            with torch.no_grad():
                                ...
                                ev = restoration_losses(x_tgt_pred, x_tgt, args)
                                mse_v = ev["mse"].item()
                                l_mse.append(mse_v)
                                l_psnr.append(psnr_neg1_to_1(mse_v))
                                l_ssim.append(ev["ssim_metric"].item())

                        logs["val/mse"] = np.mean(l_mse)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to a YAML config. Values become argparse defaults; CLI flags override.",
    )
    # args for the loss function（复原：L2→PSNR，(1-SSIM)→SSIM）
    parser.add_argument("--lambda_l2", default=1.0, type=float, help="MSE in [-1,1], optimizes PSNR.")
    parser.add_argument("--lambda_ssim", default=0.5, type=float, help="Weight for (1 - SSIM) on [0,1].")
    parser.add_argument(
        "--stochastic_forward",
        action="store_true",
        help="",
    ...
    # using the relative pose as a prompt or not
    parser.add_argument("--use_relative_pose", action="store_true",)
    # resume
    parser.add_argument("--resume", default=None, type=str)
    pre_args, _ = parser.parse_known_args()
    
    # loading default value from the config file
