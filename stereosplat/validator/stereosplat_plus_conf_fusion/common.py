"""Shared helpers for conf-fusion evaluation validators."""
...
def stereosplat_root_from_validator(file: str | Path, depth: int = 3) -> Path:
    """Resolve stereosplat repo root from validator/.../two-stage/<script>.py."""
    return Path(file).resolve().parents[depth]


def setup_import_paths(validator_file: str | Path, depth: int = 3) -> Path:
    root = stereosplat_root_from_validator(validator_file, depth=depth)
...
def load_state_dict_any(path: str, map_location: str = "cpu") -> dict:
    if path is None or str(path).strip() == "":
        raise ValueError("stage_1_model_path is empty.")
