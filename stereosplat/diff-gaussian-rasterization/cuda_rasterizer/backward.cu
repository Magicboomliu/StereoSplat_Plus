...
renderCUDA(
	...
	const float* __restrict__ geom_confs,
	const float* __restrict__ alphas,
	const uint32_t* __restrict__ n_contrib,
	const float* __restrict__ dL_dpixels,
	const float* __restrict__ dL_dpixel_depths,
	const float* __restrict__ dL_dalphas,
	const float* __restrict__ dL_dpixel_confs,
...
)
{
	...
	__shared__ float collected_confs[BLOCK_SIZE];
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		if (range.x + progress < range.y)
		{
			...
			collected_confs[block.thread_rank()] = geom_confs[coll_id];
		}
		block.sync();
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			// Propagate gradients from pixel conf to per-Gaussian conf and opacity
			const float c_conf = collected_confs[j];
			accum_conf_rec = last_alpha * last_conf + (1.f - last_alpha) * accum_conf_rec;
			last_conf = c_conf;
			dL_dopa += (c_conf - accum_conf_rec) * dL_dpixel_conf;
			atomicAdd(&(dL_dconfs[global_id]), dchannel_dcolor * dL_dpixel_conf);
...
void BACKWARD::render(
	...
	const float* colors,
	const float* depths,
	const float* geom_confs,
	const float* alphas,
	const uint32_t* n_contrib,
	const float* dL_dpixels,
	const float* dL_dpixel_depths,
	const float* dL_dalphas,
	const float* dL_dpixel_confs,
	float3* dL_dmean2D,
	float4* dL_dconic2D,
	float* dL_dopacity,
	float* dL_dcolors,
