#!/usr/bin/env bash
# Shared defaults for pixel-level conf fusion evaluation scripts.
# Source from sibling .sh files:  source "$(dirname "${BASH_SOURCE[0]}")/_common.sh"
...
# Usage: _conf_fusion_launch <gpu_yaml> <validator_rel_path> <output_folder> <extra args...>
_conf_fusion_launch() {
  local gpu_yaml="$1"
  ...
  accelerate_config_path="${STEREOSPLAT_ROOT}/accelerate_configs/inference/${gpu_yaml}"
