...
# gs_and_pixel_fusion_pose_injection_frozen_stage1_two_models.sh
# 【Stage2 + 冻结 Stage1】GS 体素融合 + Pixel conf 融合（两阶段）
#   1) 3D: G_stage1 vs G_stage2 → G_gs_fused
#   2) 2D: G_stage1 渲染 vs G_gs_fused 渲染 → 逐像素 conf 融合（A1 margin）
...
output_folder="/data1/zliu/IROS26/stereosplat_ablations/withconf/stage2/stereosplat_plus/seperated-model/fusion/gs-plus-pixel/${gs_fusion_conf_agg}/base_thresh_${gs_fusion_base_conf_thresh}/gs_margin_${gs_fusion_margin}/pixel_margin_${conf_fusion_margin}/voxel_${gs_fusion_voxel_size}/0.5_1.0"
