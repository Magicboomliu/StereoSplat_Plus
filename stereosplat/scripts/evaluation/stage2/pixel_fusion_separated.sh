...
# Stage2 | pixel_fusion | separated | toggle fusion on/off

source "$(dirname "${BASH_SOURCE[0]}")/../_common.sh"

RESULTS_BASE="/data1/zliu/IROS26/Compared_With_Others_Pixi/results/with_conf/stage2/pixel_fusion/separated"
...
eval_stage2_pixel_fusion_separated_deactivate() {
  ...
  _eval_config_stage2
  _eval_export_env
  _eval_launch gpu_0.yaml stage2 pixel_fusion separated \
...
eval_stage2_pixel_fusion_separated_activate() {
  ...
  _eval_config_stage2
  _eval_export_env
  _eval_launch gpu_1.yaml stage2 pixel_fusion separated \
