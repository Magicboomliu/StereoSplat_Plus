...
from .utilsdir.gaussain_fusion import fuse_gaussians_by_voxel_conf_margin_batched
...
# Non-interleaved train/val render layout [B,6,3,H,W] (save_val_results / train val PSNR):
#   index 0,2 → center L/R; 1,3 → last L/R; 4,5 → first L/R
...
def stereosplat_rgb_depth_eval_stats(
    rendered_images_stereo,
    rendered_depth_stereo,
    gt_images_stereo,
    gt_depth_stereo,
):
    """RGB/Depth metrics for interleaved stereo multiview renders [B, V*2, ...]."""
    rendered_images_first_stereo = rendered_images_stereo[:, -2:, :, :, :]
    gt_images_first_stereo = gt_images_stereo[:, -2:, :, :, :]
    renderded_depth_first_stereo = rendered_depth_stereo[:, -2:, :, :]
    gt_depth_first_stereo = gt_depth_stereo[:, -2:, :, :]

    rendered_images_last_stereo = rendered_images_stereo[:, -4:-2, :, :, :]
    gt_images_last_stereo = gt_images_stereo[:, -4:-2, :, :, :]
    renderded_depth_last_stereo = rendered_depth_stereo[:, -4:-2, :, :]
    gt_depth_last_stereo = gt_depth_stereo[:, -4:-2, :, :]

    rendered_images_center_stereo = rendered_images_stereo[:, -6:-4, :, :, :]
    gt_images_center_stereo = gt_images_stereo[:, -6:-4, :, :, :]
    renderded_depth_center_stereo = rendered_depth_stereo[:, -6:-4, :, :]
    gt_depth_center_stereo = gt_depth_stereo[:, -6:-4, :, :]

    rendered_images_all_stereo = rendered_images_stereo
    gt_images_all_stereo = gt_images_stereo
    renderded_depth_all_stereo = rendered_depth_stereo
    gt_depth_all_stereo = gt_depth_stereo

    first_rgb_eval_info = metrics_mean(
        pred=rendered_images_first_stereo, gt=gt_images_first_stereo)
    center_rgb_eval_info = metrics_mean(
        pred=rendered_images_center_stereo, gt=gt_images_center_stereo)
    last_rgb_eval_info = metrics_mean(
        pred=rendered_images_last_stereo, gt=gt_images_last_stereo)
    all_rgb_eval_info = metrics_mean(
        pred=rendered_images_all_stereo, gt=gt_images_all_stereo)
    ...
    return {
        "RGB": {
            "first_view_psnr_average": first_rgb_eval_info['psnr'].data.item(),
            "first_view_ssim_average": first_rgb_eval_info['ssim'].data.item(),
            "first_view_lpips_average": first_rgb_eval_info['lpips'].data.item(),
            "center_view_psnr_average": center_rgb_eval_info['psnr'].data.item(),
            "center_view_ssim_average": center_rgb_eval_info['ssim'].data.item(),
            "center_view_lpips_average": center_rgb_eval_info['lpips'].data.item(),
            "last_view_psnr_average": last_rgb_eval_info['psnr'].data.item(),
            "last_view_ssim_average": last_rgb_eval_info['ssim'].data.item(),
            "last_view_lpips_average": last_rgb_eval_info['lpips'].data.item(),
            "all_view_psnr_average": all_rgb_eval_info['psnr'].data.item(),
            "all_view_ssim_average": all_rgb_eval_info['ssim'].data.item(),
            "all_view_lpips_average": all_rgb_eval_info['lpips'].data.item(),
        },
        "Depth": {
            "first_view_Abs_Rel_average": first_view_depth_eval_info['AbsRel'].data.item(),
            "frist_view_Sq_Rel_average": first_view_depth_eval_info['SqRel'].data.item(),
            "first_view_RMSE_log_average": first_view_depth_eval_info['RMSE_log'].data.item(),
            "center_view_Abs_Rel_average": center_view_depth_eval_info['AbsRel'].data.item(),
            "center_view_Sq_Rel_average": center_view_depth_eval_info['SqRel'].data.item(),
            "center_view_RMSE_log_average": center_view_depth_eval_info['RMSE_log'].data.item(),
            "last_view_Abs_Rel_average": last_view_depth_eval_info['AbsRel'].data.item(),
            "last_view_Sq_Rel_average": last_view_depth_eval_info['SqRel'].data.item(),
            "last_view_RMSE_log_average": last_view_depth_eval_info['RMSE_log'].data.item(),
            "all_view_Abs_Rel_average": all_view_depth_eval_info['AbsRel'].data.item(),
            "all_view_Sq_Rel_average": all_view_depth_eval_info['SqRel'].data.item(),
            "all_view_RMSE_log_average": all_view_depth_eval_info['RMSE_log'].data.item(),
        },
    }


def fuse_renders_by_conf_pixelwise(
    rgb_a,
    rgb_b,
    depth_a,
    depth_b,
    ...
    """Per-pixel fusion: pick render B where conf_b beats conf_a, else render A.

    Args:
        rgb_a/rgb_b: [B, V, 3, H, W]
        depth_a/depth_b: [B, V, H, W]
        conf_a/conf_b: [B, V, H, W] or None
        conf_fusion_margin: None = legacy ``conf_b >= conf_a`` (ties prefer B/plus).
            float = ``conf_b > conf_a + margin`` (ties and small gaps prefer A/base).
    """
    ...
    if conf_fusion_margin is None:
        pick_b = conf_b >= conf_a  # legacy: ties prefer B (plus / stage2)
    else:
        pick_b = conf_b > conf_a + conf_fusion_margin  # A1: ties prefer A (base)
...
def _build_per_view_fusion_margins(
    num_views,
    device,
    first_margin,
    center_margin,
    ...
    """Per-view margin tensor for KITTI-360 6-view render layout (pre-interleave).

    V order: [center_L, last_L, center_R, last_R, first_L, first_R].
    """
    margins = torch.zeros(1, num_views, 1, 1, device=device, dtype=torch.float32)
    if num_views == 6:
        margins[0, 0, 0, 0] = center_margin
        margins[0, 1, 0, 0] = last_margin
        margins[0, 2, 0, 0] = center_margin
        margins[0, 3, 0, 0] = last_margin
        margins[0, 4, 0, 0] = first_margin
        margins[0, 5, 0, 0] = first_margin
    else:
        margins.fill_(center_margin)
    return margins


def _calibrate_conf_maps(conf_a, conf_b, calibration):
...
def fuse_renders_per_view_adaptive(
    ...
    """Per-view adaptive fusion: different margins / calibration per trajectory view."""
    if conf_a is None and conf_b is None:
        return rgb_b, depth_b, None
    if conf_a is None:
        return rgb_b, depth_b, conf_b
    ...
    _, num_views, _, _ = conf_a.shape
    margins = _build_per_view_fusion_margins(
        num_views,
        conf_a.device,
        first_margin,
        center_margin,
    ...
    conf_a_cal, conf_b_cal = _calibrate_conf_maps(conf_a, conf_b, calibration)
    ...
    if temperature is None:
        ...
        conf_diff = conf_b_cal - conf_a_cal - margins
...
def pixel_fuse_renders(
    ...
    return fuse_renders_by_conf_pixelwise(
        rgb_a,
        rgb_b,
        depth_a,
        depth_b,
...
class StereoSplat(BaseModule):
    ...
    def prepare_input_multiview(self,
        ...
        # 3→4 imgs, 5→6 imgs; only 2 uses one stereo pair.
        stereo_pairs_nums = (view_num + 1) // 2
    ...
    def forward_to_3dgs(self,input_batch_dict,cfg=None):
    ...
    def prepare_input_multiview_stage2(self,
                                batch,
                                view_num=2,
                                matching_nums=2):
        ...
        input_batch_dict_for_psuedo_view_rendering = dict()
        ...
        # Used for Pusedo View Mix Training Input Information.
        input_camera_c2w_for_psuedo_view_rendering = batch["input_info_for_psuedo_view_rendering"]["c2w"]
        input_camera_fovx_for_psuedo_view_rendering = batch["input_info_for_psuedo_view_rendering"]["fovx"]
        input_camera_fovy_for_psuedo_view_rendering = batch["input_info_for_psuedo_view_rendering"]["fovy"]
        ...
        # used for the psuedo view rendering index
        input_camera_c2w_for_psuedo_view_rendering = input_camera_c2w_for_psuedo_view_rendering[:,index,:,:]
        input_camera_fovx_for_psuedo_view_rendering = input_camera_fovx_for_psuedo_view_rendering[:,index]
        input_camera_fovy_for_psuedo_view_rendering = input_camera_fovy_for_psuedo_view_rendering[:,index]
        ...
        # input dict 
        input_batch_dict_for_psuedo_view_rendering['c2w'] = input_camera_c2w_for_psuedo_view_rendering.to(device_id, dtype=self.dtype)
        ...
        return input_batch_dict,input_batch_dict_for_psuedo_view_rendering, output_batch_dict


    def forward_stage2(self,batch,
                mode='train',
                view_num=4,
                matching_nums=3,            
                iter=0,
    ...
    def forward_stage2_with_difix3d(self,batch,
                mode='train',
                view_num=4,
                matching_nums=3,            
                iter=0,
        ...
        # Difix3D can have its own ratio (default: same as pseudo ratio for back-compat)
        _difix3d_ratio = mix_difix3d_ratio if mix_difix3d_ratio is not None else mix_psuedo_views_ratio
        ...
        if pseudo_source_model is not None and use_mix_psuedo_views <= mix_psuedo_views_ratio:
            ...
            if pretrained_diffix_model is not None and use_mix_psuedo_difix3d_views <= _difix3d_ratio:
                # reference views
                ...
                for i in range(psuedo_input_views.shape[1]):
                    if i%2 == 0:
                        current_reference_view = left_image_ref_pil
                    else:
                        current_reference_view = right_image_ref_pil
                    
                    current_psuedo_view = psuedo_input_views[0,i,:,:,:]
                    current_psuedo_view = current_psuedo_view.permute(1,2,0).cpu().numpy()
                    current_psuedo_view = (current_psuedo_view*255).astype(np.uint8)
                    current_psuedo_view_pil = Image.fromarray(current_psuedo_view)
                    
                    enhanced_psuedo_view = pretrained_diffix_model.sample(
                        current_psuedo_view_pil,
                        height=112,
                        width=544,
                        ref_image=current_reference_view,
                        prompt=cfg.prompt
                    )
                    
                    enhanced_psuedo_view = convert_pil_to_tensor(enhanced_psuedo_view)
                    psuedo_input_views[0,i,:,:,:] = enhanced_psuedo_view
            


            # update the input batch into  mix training
            input_batch_dict['imgs'][:,2:,:,:,:] = psuedo_input_views[:,2:,:,:,:]
            # 4 or 6 input views (view_num 3/4 or 5/6) — pseudo replaces imgs[:,2:].
            _did_mix_pseudo = view_num >= 3
        ...
        if mode=='train' or mode=='val':
            ...
            _fsd = self.losses_params.fusion_sup_dict
            _w_2v_floor = getattr(_fsd, 'weight_2v_floor', 0.0)
            _w_2v_ceil = getattr(_fsd, 'weight_2v_ceiling', 0.0)
            _w_2v_floor_mv = getattr(_fsd, 'weight_2v_floor_mv', 0.0)
            _legacy_2v_distill = getattr(_fsd, 'weight_2v_distill', 0.0)
            if _w_2v_floor == 0.0 and _w_2v_ceil == 0.0 and _legacy_2v_distill > 0.0:
                _w_2v_floor = _legacy_2v_distill
                _w_2v_ceil = _legacy_2v_distill * 0.6
            # Only view_num=2 has 2 input images; view_num=3 uses 4 (same as 4).
            _use_2v_anchor_only = (
            ...
            def _render_frozen_stage1_2v(_ref_model):
                with torch.no_grad():
                    _in2v_r, _ = _ref_model.prepare_input_multiview(
                        batch=batch, view_num=2, matching_nums=2)
                    _f2v_r = _ref_model.extract_img_feat(img=_in2v_r["imgs"])
                    _gcv_r, _gfeat_r, _ = _ref_model.costvolume_gs(
                        _in2v_r, cfg=cfg, images_feat=_f2v_r[0])
                    _pck_r = _ref_model.dataset_params.pc_range
                    _x0r,_y0r,_z0r,_x1r,_y1r,_z1r = _pck_r
                    _cmr, _fmr = [], []
                    for _br in range(_gcv_r.shape[0]):
                        _mr = (_gcv_r[_br,:,0]>=_x0r)&(_gcv_r[_br,:,0]<=_x1r)&\
                              (_gcv_r[_br,:,1]>=_y0r)&(_gcv_r[_br,:,1]<=_y1r)&\
                              (_gcv_r[_br,:,2]>=_z0r)&(_gcv_r[_br,:,2]<=_z1r)
                        _cmr.append(_gcv_r[_br][_mr])
                        _fmr.append(_gfeat_r[_br][_mr])
                    _gvol_r = _ref_model.volume_gs(
                        [_f2v_r[0]], _in2v_r['extrinsics'], _cmr, _fmr, _in2v_r['img_metas'])
                    _gcv_r  = sanitize_gaussians_tensor(_gcv_r)
                    _gvol_r = sanitize_gaussians_tensor(_gvol_r)
                    _gall_r = torch.cat([_gcv_r, _gvol_r], dim=1)
                    _pkg_r  = _ref_model.renderer.render(
                        gaussians=_gall_r, c2w=render_c2w,
                        fovx=render_fovxs, fovy=render_fovys, rays_o=None, rays_d=None)
                    return torch.clamp(_pkg_r['image'], 0.0, 1.0)
            ...
            if (_run_fusion_train or _use_2v_anchor_mv):
                ...
                # Grad 2v pass must disable activation checkpoint: recomputation in eval()
                # vs train() (or 6v then 2v) causes CheckpointError shape mismatches.
                ...
                if view_num >= 5 and torch.cuda.is_available():
                ...
                try:
                    with _2v_ctx:
                        ...
                        del _g_cv_2v, _g_vol_2v, _g_feat_2v, _feats2v
                ...
                if _run_fusion_train and _conf_2v is not None:
                    ...
                    if _w_conf_comp > 0 and _did_mix_pseudo and rendered_conf_fuse is not None and _conf_2v is not None:
                        with torch.no_grad():
                            ...
                            _err_mv = (rgb_gt - rendered_color_fuse).pow(2).mean(dim=2)
                        _err_delta = (_err_mv - _err_2v).detach()
                        _loss_conf_comp = torch.nn.functional.relu(
                            (rendered_conf_fuse - _conf_2v.detach()) * _err_delta
                        ).mean()
                    ...
                    if _w_mv_margin > 0 and _conf_2v is not None:
                        ...
                        _psnr_2v_m_avg = (-10.0 * torch.log10(
                            _err_2v_pv_m + 1e-8)).mean()
                        _loss_mv_margin = torch.nn.functional.relu(
                    ...
                    # center/last per-view margin hinges (weight_margin_key_views; aligns w/ val layout)
            ...
            if _use_2v_anchor_only:
                ...
                _err_s1  = (rgb_gt - _rgb_s1).pow(2)
                if _w_2v_floor > 0.0:
                    _loss_2v_floor = torch.nn.functional.relu(
                        _err_cur - _err_s1.detach()).mean()
                    fusion_branch_loss = fusion_branch_loss + _w_2v_floor * _loss_2v_floor
                    set_loss("2v_floor", mode, _loss_2v_floor, _w_2v_floor)
                if _w_2v_ceil > 0.0:
                    _loss_2v_ceil = torch.nn.functional.relu(
                        _err_s1.detach() - _err_cur).mean()
                    fusion_branch_loss = fusion_branch_loss + _w_2v_ceil * _loss_2v_ceil
                    set_loss("2v_ceiling", mode, _loss_2v_ceil, _w_2v_ceil)
            ...
            if mode == 'val' and rendered_color_fuse is not None and rendered_conf_fuse is not None:
                with torch.no_grad():
                    ...
                    _pc = self.dataset_params.pc_range
                    _x0, _y0, _z0, _x1, _y1, _z1 = _pc
                    _cv_mask2v_val, _feat_mask2v_val = [], []
                    for _b in range(_g_cv_2v_val.shape[0]):
                        _m = (_g_cv_2v_val[_b,:,0] >= _x0) & (_g_cv_2v_val[_b,:,0] <= _x1) & \
                             (_g_cv_2v_val[_b,:,1] >= _y0) & (_g_cv_2v_val[_b,:,1] <= _y1) & \
                             (_g_cv_2v_val[_b,:,2] >= _z0) & (_g_cv_2v_val[_b,:,2] <= _z1)
                        _cv_mask2v_val.append(_g_cv_2v_val[_b][_m])
                        _feat_mask2v_val.append(_g_feat_2v_val[_b][_m])
                    _g_vol_2v_val = self.volume_gs(
                        [_feats2v_val[0]], _in2v_val['extrinsics'],
                        _cv_mask2v_val, _feat_mask2v_val, _in2v_val['img_metas'])
                    _g_cv_2v_val  = sanitize_gaussians_tensor(_g_cv_2v_val)
                    _g_vol_2v_val = sanitize_gaussians_tensor(_g_vol_2v_val)
                    _g_all_2v_val = torch.cat([_g_cv_2v_val, _g_vol_2v_val], dim=1)
                    _pkg_2v_val = self.renderer.render(
                        gaussians=_g_all_2v_val,
                        c2w=render_c2w, fovx=render_fovxs, fovy=render_fovys,
                        rays_o=None, rays_d=None,
                    )
                    _rgb_2v_val  = torch.clamp(_pkg_2v_val['image'], 0.0, 1.0)
                    _conf_2v_val = conf_from_render_pkg(_pkg_2v_val)

                    _gt_f = rgb_gt.clamp(0.0, 1.0)

                    def _psnr_stereo_avg(pred, gt, left_i, right_i):
                        _mse_l = (pred[:, left_i] - gt[:, left_i]).pow(2).mean()
                        _mse_r = (pred[:, right_i] - gt[:, right_i]).pow(2).mean()
                        _mse = (_mse_l + _mse_r) * 0.5
                        return (-10.0 * torch.log10(_mse + 1e-8)).item()

                    # Output tensor layout (matches save_val_results / dataloader):
                    #   index 0,2 → center left/right
                    #   index 1,3 → last   left/right
                    #   index 4,5 → first  left/right
                    _vi_lr = {
                        'center': (0, 2),
                        'last': (1, 3),
                        'first': (4, 5),
                    ...
                    # multiview-only PSNR (no pixel fusion)
                    _mv = rendered_color_fuse.clamp(0.0, 1.0)
                    loss_terms['val/psnr_multiview_center'] = _psnr_stereo_avg(
                        _mv, _gt_f, *_vi_lr['center'])
                    loss_terms['val/psnr_multiview_last'] = _psnr_stereo_avg(
                        _mv, _gt_f, *_vi_lr['last'])
                    loss_terms['val/psnr_multiview_first'] = _psnr_stereo_avg(
                        _mv, _gt_f, *_vi_lr['first'])
                    loss_terms['val/psnr_multiview_mean'] = sum(
                        _psnr_stereo_avg(_mv, _gt_f, *_vi_lr[name])
                        for name in ('center', 'last', 'first')) / 3.0
                    ...
                    if _conf_2v_val is not None:
                        ...
                        loss_terms['val/psnr_fused_center'] = _psnr_stereo_avg(
                            _fv, _gt_f, *_vi_lr['center'])
                        loss_terms['val/psnr_fused_last'] = _psnr_stereo_avg(
                            _fv, _gt_f, *_vi_lr['last'])
                        loss_terms['val/psnr_fused_first'] = _psnr_stereo_avg(
                            _fv, _gt_f, *_vi_lr['first'])
                        loss_terms['val/psnr_fused_mean'] = sum(
                            _psnr_stereo_avg(_fv, _gt_f, *_vi_lr[name])
                            for name in ('center', 'last', 'first')) / 3.0
    ...
    def forward_kitti360_videos(self,
                                ...
                                view_num=4,
                                matching_nums=3,
                                pseudo_ratio_index=None):

        if pseudo_ratio_index is not None:
            ...
            gaussians_all = self._build_gaussians_all_from_input(input_batch_dict, cfg)
    ...
    def _build_gaussians_all_from_input(self, input_batch_dict, cfg):
        """costvolume_gs + volume_gs → gaussians_all [B,N,D]."""
        img = input_batch_dict["imgs"]
        img_feats = self.extract_img_feat(img=img)
        gaussians_cv, gaussians_feat, pred_depths = self.costvolume_gs(
            input_batch_dict, cfg=cfg, images_feat=img_feats[0]
        ...
        gaussians_cv = sanitize_gaussians_tensor(gaussians_cv)
        gaussians_volume = sanitize_gaussians_tensor(gaussians_volume)
        return torch.cat([gaussians_cv, gaussians_volume], dim=1)

    def _render_trajectory_from_gaussians(self, gaussians_all, output_batch_dict):
        render_pkg = self.renderer.render(
            gaussians=gaussians_all,
            c2w=output_batch_dict["output_c2ws"],
            fovx=output_batch_dict["output_fovxs"],
            fovy=output_batch_dict["output_fovys"],
            rays_o=None,
            rays_d=None,
        )
        color = torch.clamp(render_pkg["image"], min=0, max=1.0)
        depth = torch.clamp(render_pkg["depth"].squeeze(2), min=0, max=150)
        return color, depth, conf_from_render_pkg(render_pkg)

    def _pseudo_stereo_frames_from_base_render(
        self,
        rendered_images_fusion,
        pseudo_ratio_index,
        *,
        ...
        """从 G_base 全轨迹渲染中按 pseudo_ratio 取出 4 张 pseudo stereo（与 S+ 一致）。"""
        rendered_images_first_stereo = rendered_images_fusion[:, -2:, :, :, :]
        rendered_images_last_stereo = rendered_images_fusion[:, -4:-2, :, :, :]
        rendered_images_center_stereo = rendered_images_fusion[:, -6:-4, :, :, :]
        ...
        if pseudo_ratio_index[0] == 0.5 and pseudo_ratio_index[1] == 1.0:
            ...
            pair_nums = raw_rendered_rest_images.shape[1] // 2
            s_l = int(pair_nums * pseudo_ratio_index[0]) * 2
            t_l = int(pair_nums * pseudo_ratio_index[1]) * 2
            second = raw_rendered_rest_images[:, s_l : s_l + 2, :, :, :]
            third = raw_rendered_rest_images[:, t_l : t_l + 2, :, :, :]
            pseudo_stereo_frames = torch.cat([second, third], dim=1)

        if not use_diffix3d or diffix3d_network is None or input_batch_dict is None:
            return pseudo_stereo_frames

        ref_left = input_batch_dict["imgs"][0, 0, :, :, :].permute(1, 2, 0).cpu().numpy()
        ref_right = input_batch_dict["imgs"][0, 1, :, :, :].permute(1, 2, 0).cpu().numpy()
        ref_left_pil = Image.fromarray((ref_left * 255).astype(np.uint8))
        ref_right_pil = Image.fromarray((ref_right * 255).astype(np.uint8))
        enhanced_views = []
        for view_idx in range(pseudo_stereo_frames.shape[1]):
            raw_view = pseudo_stereo_frames[0, view_idx, :, :, :].permute(1, 2, 0).cpu().numpy()
            raw_view_pil = Image.fromarray((raw_view * 255).astype(np.uint8))
            ref_pil = ref_left_pil if view_idx % 2 == 0 else ref_right_pil
            enhanced_view_pil = diffix3d_network.sample(
                raw_view_pil, height=112, width=544, ref_image=ref_pil, prompt=cfg.prompt
            )
            enhanced_views.append(convert_pil_to_tensor(enhanced_view_pil))
        return torch.cat(enhanced_views, dim=1)

    def _eval_interleaved_stereo_buckets(self, rendered_images, rendered_depth, gt_images, gt_depth):
        buckets = {}
        for name, img_sl, dep_sl in [
            ("first", slice(-2, None), slice(-2, None)),
            ("center", slice(-6, -4), slice(-6, -4)),
            ("last", slice(-4, -2), slice(-4, -2)),
            ("all", slice(None), slice(None)),
        ]:
            buckets[name] = {
                "rgb": metrics_mean(rendered_images[:, img_sl], gt_images[:, img_sl]),
                "depth": depth_metrics_absrel_sqrel_rmse_log(
                    rendered_depth[:, dep_sl], gt_depth[:, dep_sl]
                ),
            }
        rgb_stat = {
            f"{k}_view_psnr_average": buckets[k]["rgb"]["psnr"].data.item()
            for k in ("first", "center", "last", "all")
        }
        rgb_stat.update(
            {
                f"{k}_view_ssim_average": buckets[k]["rgb"]["ssim"].data.item()
                for k in ("first", "center", "last", "all")
            }
        )
        rgb_stat.update(
            {
                f"{k}_view_lpips_average": buckets[k]["rgb"]["lpips"].data.item()
                for k in ("first", "center", "last", "all")
            }
        )
        depth_stat = {
            "first_view_Abs_Rel_average": buckets["first"]["depth"]["AbsRel"].data.item(),
            "frist_view_Sq_Rel_average": buckets["first"]["depth"]["SqRel"].data.item(),
            "first_view_RMSE_log_average": buckets["first"]["depth"]["RMSE_log"].data.item(),
            "center_view_Abs_Rel_average": buckets["center"]["depth"]["AbsRel"].data.item(),
            "center_view_Sq_Rel_average": buckets["center"]["depth"]["SqRel"].data.item(),
            "center_view_RMSE_log_average": buckets["center"]["depth"]["RMSE_log"].data.item(),
            "last_view_Abs_Rel_average": buckets["last"]["depth"]["AbsRel"].data.item(),
            "last_view_Sq_Rel_average": buckets["last"]["depth"]["SqRel"].data.item(),
            "last_view_RMSE_log_average": buckets["last"]["depth"]["RMSE_log"].data.item(),
            "all_view_Abs_Rel_average": buckets["all"]["depth"]["AbsRel"].data.item(),
            "all_view_Sq_Rel_average": buckets["all"]["depth"]["SqRel"].data.item(),
            "all_view_RMSE_log_average": buckets["all"]["depth"]["RMSE_log"].data.item(),
        }
        return rgb_stat, depth_stat, buckets
    ...
    def infer_oracle_upper_bound_ablation(self,
        ...
        1) 2-view → G_base → 全轨迹渲染
        2) 按 pseudo_ratio 取 pseudo 渲染图 → reinject → G_plus → 全轨迹渲染
        3) 用 GT 逐像素比较 G_base / G_plus 的 RGB 误差，选更小者融合（非 conf 融合）
        """
        if not pseudo_ratio_index or len(pseudo_ratio_index) < 2:
            pseudo_ratio_index = [0.5, 1.0]
        ...
        with torch.no_grad():
            input_2view, output_2view = self.prepare_input_multiview(
                batch=batch, view_num=view_num, matching_nums=matching_nums
            )
            gaussians_all_base = self._build_gaussians_all_from_input(input_2view, cfg)
            base_color, base_depth, _ = self._render_trajectory_from_gaussians(
                gaussians_all_base, output_2view
            )

            rendered_images_base = interleave_left_right(base_color)
            rendered_depth_base = interleave_left_right_depth(base_depth)

            input_batch_dict, output_batch_dict = self.prepare_tripleview_by_ratio_index(
                batch=batch,
                pseudo_ratio_index=pseudo_ratio_index,
            )
            pseudo_stereo_frames = self._pseudo_stereo_frames_from_base_render(
                rendered_images_base,
                pseudo_ratio_index,
            ...
            gaussians_all_plus = self._build_gaussians_all_from_input(input_batch_dict, cfg)
            plus_color, plus_depth, _ = self._render_trajectory_from_gaussians(
                gaussians_all_plus, output_batch_dict
            )

        rendered_images_plus = interleave_left_right(plus_color)
        rendered_depth_plus = interleave_left_right_depth(plus_depth)
        ...
        fusion_depth = (
            rendered_depth_base * base_mask.squeeze(2)
            + rendered_depth_plus * plus_mask.squeeze(2)
        )

        fusion_rgb_stat, fusion_depth_stat, fusion_buckets = self._eval_interleaved_stereo_buckets(
            fusion_image, fusion_depth, rendered_images_gt, sparse_depth_gt
        )
        base_rgb_stat, base_depth_stat, _ = self._eval_interleaved_stereo_buckets(
            rendered_images_base, rendered_depth_base, rendered_images_gt, sparse_depth_gt
        )
        plus_rgb_stat, plus_depth_stat, _ = self._eval_interleaved_stereo_buckets(
            rendered_images_plus, rendered_depth_plus, rendered_images_gt, sparse_depth_gt
        )

        evaluation_results_stat = {
            "RGB": fusion_rgb_stat,
            "Depth": fusion_depth_stat,
            "Oracle_reference": {
                "G_base_all_view_psnr": base_rgb_stat["all_view_psnr_average"],
                "G_base_all_view_ssim": base_rgb_stat["all_view_ssim_average"],
                "G_base_all_view_lpips": base_rgb_stat["all_view_lpips_average"],
                "G_base_all_view_Abs_Rel": base_depth_stat["all_view_Abs_Rel_average"],
                "G_plus_all_view_psnr": plus_rgb_stat["all_view_psnr_average"],
                "G_plus_all_view_ssim": plus_rgb_stat["all_view_ssim_average"],
                "G_plus_all_view_lpips": plus_rgb_stat["all_view_lpips_average"],
                "G_plus_all_view_Abs_Rel": plus_depth_stat["all_view_Abs_Rel_average"],
                "G_fusion_all_view_psnr": fusion_rgb_stat["all_view_psnr_average"],
                "G_fusion_all_view_ssim": fusion_rgb_stat["all_view_ssim_average"],
                "G_fusion_all_view_lpips": fusion_rgb_stat["all_view_lpips_average"],
                "G_fusion_all_view_Abs_Rel": fusion_depth_stat["all_view_Abs_Rel_average"],
            },
        }

        if vis:
            saved_folder = os.path.join(val_result_savedir, bin_token_name)
            os.makedirs(saved_folder, exist_ok=True)

            def _save_stereo_triplet(folder_img, folder_dep, images, depths):
                os.makedirs(folder_img, exist_ok=True)
                os.makedirs(folder_dep, exist_ok=True)
                for name, isl, dsl in [
                    ("first", slice(-2, None), slice(-2, None)),
                    ("center", slice(-6, -4), slice(-6, -4)),
                    ("last", slice(-4, -2), slice(-4, -2)),
                ]:
                    pred_img = torch.cat((images[:, isl][:, 0], images[:, isl][:, 1]), dim=-1)
                    skimage.io.imsave(
                        os.path.join(folder_img, f"{name}_stereo.png"),
                        (pred_img.squeeze(0).permute(1, 2, 0).cpu().numpy() * 255).astype(np.uint8),
                    )
                    pred_d = torch.cat((depths[:, dsl][:, 0], depths[:, dsl][:, 1]), dim=-1)
                    skimage.io.imsave(
                        os.path.join(folder_dep, f"{name}_stereo_depth.png"),
                        convert_depth_to_disp(pred_d.squeeze(0).cpu().numpy()),
                    )

            _save_stereo_triplet(
                os.path.join(saved_folder, "GT Images"),
                os.path.join(saved_folder, "GT Depth"),
                rendered_images_gt,
                sparse_depth_gt,
            )
            _save_stereo_triplet(
                os.path.join(saved_folder, "rendered_fusion"),
                os.path.join(saved_folder, "rendered_fusion_depth"),
                fusion_image,
                fusion_depth,
            )
            _save_stereo_triplet(
                os.path.join(saved_folder, "rendered_G_base"),
                os.path.join(saved_folder, "rendered_G_base_depth"),
                rendered_images_base,
                rendered_depth_base,
            )
            _save_stereo_triplet(
                os.path.join(saved_folder, "rendered_G_plus"),
                os.path.join(saved_folder, "rendered_G_plus_depth"),
    ...
    def infer_pose_injection_frozen_stage1_two_models(self,
        ...
        rendered_color_gs_fused = None
        rendered_depth_gs_fused = None
        rendered_conf_gs_fused = None
        rendered_color_stage2 = None
        rendered_depth_stage2 = None
        rendered_conf_stage2 = None

        if gs_conf_fusion:
            pc_range = self.dataset_params.pc_range
            if hasattr(pc_range, "tolist"):
                pc_range = pc_range.tolist()
            else:
                pc_range = list(pc_range)
            gaussians_for_render, _ = fuse_gaussians_by_voxel_conf_margin_batched(
                gaussians_all_stage1,
                gaussians_all_stage2,
                point_cloud_range=pc_range,
                voxel_size=gs_fusion_voxel_size,
                conf_fusion_margin=gs_fusion_margin,
                conf_agg=gs_fusion_conf_agg,
                base_conf_thresh=gs_fusion_base_conf_thresh,
            )

            render_pkg_gs_fused = self.renderer.render(
                gaussians=gaussians_for_render,
                c2w=render_c2w,
                fovx=render_fovxs,
                fovy=render_fovys,
                rays_o=None,
            ...
            rendered_color_gs_fused = torch.clamp(
                render_pkg_gs_fused['image'], min=0, max=1.0)
            rendered_depth_gs_fused = torch.clamp(
        ...
        if gs_conf_fusion and pixel_level_conf_fusion:
            # Reuse frozen Stage1 render from pseudo-injection (same G_stage1 & cameras).
            ...
            rendered_color_fuse = rendered_color_gs_fused
            rendered_depth_fuse = rendered_depth_gs_fused
            rendered_conf_fuse = rendered_conf_gs_fused
        ...
        if rendered_conf_stage1_final is not None:
            rendered_conf_stage1_final_fusion = interleave_left_right_depth(
                rendered_conf_stage1_final)
        rendered_conf_gs_fused_fusion = None
        if rendered_conf_gs_fused is not None:
            rendered_conf_gs_fused_fusion = interleave_left_right_depth(rendered_conf_gs_fused)
        sparse_depth_gt = interleave_left_right_depth(sparse_depth_gt)
        rendered_images_gt = interleave_left_right(output_rgb)
        ...
        if gs_conf_fusion and pixel_level_conf_fusion:
            ...
            conf_stats_gs_fused = stereosplat_conf_eval_stats(rendered_conf_gs_fused_fusion)
            if conf_stats_gs_fused is not None:
                evaluation_results_stat["Conf_gs_fused"] = conf_stats_gs_fused
    ...
    def infer_pixel_fusion_pose_injection_single_model(self,
        ...
        rendered_conf_pseudo_multiview = None
        rendered_color_gs_fused = None
        rendered_depth_gs_fused = None
        rendered_conf_gs_fused = None
        ...
        if gs_conf_fusion:
            ...
            gaussians_for_render, _ = fuse_gaussians_by_voxel_conf_margin_batched(
                gaussians_all_2view,
                gaussians_all_pseudo_multiview,
                point_cloud_range=pc_range,
                voxel_size=gs_fusion_voxel_size,
                conf_fusion_margin=gs_fusion_margin,
                conf_agg=gs_fusion_conf_agg,
        ...
        def _append_branch_stats(color, depth, rgb_key, depth_key):
            if color is None or depth is None:
                return
            images_stereo = interleave_left_right(color)
            depth_stereo = interleave_left_right_depth(depth)
            branch_stats = stereosplat_rgb_depth_eval_stats(
                images_stereo, depth_stereo, rendered_images_gt, sparse_depth_gt)
            evaluation_results_stat[rgb_key] = branch_stats["RGB"]
            evaluation_results_stat[depth_key] = branch_stats["Depth"]

        fused_stats = stereosplat_rgb_depth_eval_stats(
            rendered_images_fusion,
            rendered_depth_fusion,
            rendered_images_gt,
            sparse_depth_gt,
        )
        evaluation_results_stat = {
            "RGB": fused_stats["RGB"],
            "Depth": fused_stats["Depth"],
        }
        _append_branch_stats(
            rendered_color_2view_final,
            rendered_depth_2view_final,
            "RGB_2view",
            "Depth_2view",
        ...
        _append_branch_stats(
            rendered_color_pseudo_multiview,
            rendered_depth_pseudo_multiview,
            "RGB_pseudo_multiview",
            "Depth_pseudo_multiview",
        )
        ...
        if gs_conf_fusion and pixel_level_conf_fusion:
            ...
            if conf_stats_fused is not None:
                evaluation_results_stat["Conf"] = conf_stats_fused
            conf_stats_gs_fused = stereosplat_conf_eval_stats(
                rendered_conf_gs_fused_fusion)
            if conf_stats_gs_fused is not None:
                evaluation_results_stat["Conf_gs_fused"] = conf_stats_gs_fused
...
_LPIPS_FN_CACHE: dict[tuple[str, str], lpips.LPIPS] = {}


def _cached_lpips(net: str, device: torch.device) -> lpips.LPIPS:
    key = (net, str(device))
    if key not in _LPIPS_FN_CACHE:
        _LPIPS_FN_CACHE[key] = lpips.LPIPS(net=net).to(device).eval()
    return _LPIPS_FN_CACHE[key]
...
def lpips_mean(pred: torch.Tensor, gt: torch.Tensor, net: str = "alex") -> torch.Tensor:
    ...
    device = pred.device
    loss_fn = _cached_lpips(net, device)
...
def metrics_mean(pred: torch.Tensor, gt: torch.Tensor, lpips_net: str = "alex"):
    ...
    # LPIPS (cached — avoid reloading alex weights every metrics_mean call)
    loss_fn = _cached_lpips(lpips_net, device)
