...
try:
    # diffusers versions differ: older releases don't expose this mixin.
    from diffusers.loaders import FromOriginalVAEMixin  # type: ignore
except ImportError:  # pragma: no cover
    class FromOriginalVAEMixin:  # type: ignore
        pass
...
class AutoencoderKL(ModelMixin, ConfigMixin, FromOriginalVAEMixin):
    ...
    def __init__(
        ...
        # Some versions expose PEFT adapter helpers (add_adapter) on ModelMixin, others don't.
        # For inference-only usage, skipping adapter registration is acceptable.
        if hasattr(self, "add_adapter"):
            self.add_adapter(vae_lora_config, adapter_name="vae_skip")
        else:
            warnings.warn(
                "diffusers ModelMixin has no `add_adapter`; skipping VAE LoRA adapter registration.",
                RuntimeWarning,
            )
