...
class DifixRef(torch.nn.Module):
    def __init__(self, 
        ...
        # Some diffusers versions may initialize weights on `meta` device when low_cpu_mem_usage=True.
        # Force eager weight materialization to avoid "Cannot copy out of meta tensor" on `.to("cuda")`.
        ...
        if pretrained_path is not None:
            ...
            # Adapter registration is optional for inference; some diffusers versions don't provide it.
            if hasattr(vae, "add_adapter"):
                vae.add_adapter(vae_lora_config, adapter_name="vae_skip")
            else:
                print("Warning: VAE has no `add_adapter`; skipping adapter registration.")
