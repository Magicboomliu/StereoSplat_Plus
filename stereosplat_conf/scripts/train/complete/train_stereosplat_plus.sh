...
Train_StereoSplat_Plus_With_Conf_On_KITTI360() {
...
work_dir="${REPO}/outputs/models/all/withconf/stereosplat_plus_conf"
output_dir="${REPO}/outputs/logs/all/withconf/stereosplat_plus_conf"
...
stage_1_model_path="${STAGE1_CHECKPOINT:-/path/to/stage1_stereosplat_conf}"
...
wandb_run_name="${WANDB_RUN_NAME:-stereosplat_plus_complete}"
...
if [ ! -e "$stage_1_model_path" ]; then
  ...
  echo "        Set STAGE1_CHECKPOINT or edit stage_1_model_path in this script."
  exit 1
fi
...
if [ ! -f "$pretrained_difix3d" ]; then
  ...
  echo "        Set DIFIX3D_WEIGHTS or edit pretrained_difix3d in this script."
  exit 1
fi
