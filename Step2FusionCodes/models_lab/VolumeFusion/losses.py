...
def normalize_tensor(x, eps=1e-6):
    norm_factor = torch.sqrt(torch.sum(x ** 2, dim=1, keepdim=True).clamp(min=eps))
...
def Custom_Depth_Loss(depth_pred, depth_gt, alpha=2.0, beta=1.0):
    """
    ...
    # ✅ Safe padding for replicate mode — use 4D padding (left, right, top, bottom)
    grad_loss_x = F.pad(grad_loss_x, (0, 1, 0, 0), mode='replicate')  # pad right on W
    grad_loss_y = F.pad(grad_loss_y, (0, 0, 0, 1), mode='replicate')  # pad bottom on H
    ...
    total_loss = total_loss *1.0/(alpha+beta)
