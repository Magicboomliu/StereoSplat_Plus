...
def main(args):
    ...
    if args.dataset_type=="Center_LiDAR":
        ...
        sys.path.append("..")
        # import data.KITTI360_For_Val.KITTI360_CenterCam_Ref.dataloader as datasets
        import data.KITTI360_FirstLiDAR_Ref_Fuse.dataloader as datasets
    else:
    ...
    with torch.no_grad():
        ...
        for batch in tqdm(val_dataloader):
            ...
            '''
            "outputs": dict_keys(['rgb', 'c2w', 'fovx', 'fovy', 'rays_o', 'rays_d', 'input_image_path', 'depth', 'depth_m', 'conf_m', 'sparse_gt_depth'])
            '''
...
def swap_first_three(lst):
    lst = list(lst) 
    if len(lst) >= 3:
        lst[:3] = [lst[1], lst[0], lst[2]]
    return lst

def preprocess_the_batch_for_incremental_fuse(batch):
    '''
    ...
    batch['inputs_pix']['cx'] = swap_first_three(batch['inputs_pix']['cx'])
    batch['inputs_pix']['cy'] = swap_first_three(batch['inputs_pix']['cy'])
    batch['inputs_pix']['fx'] = swap_first_three(batch['inputs_pix']['fx'])
    batch['inputs_pix']['fy'] = swap_first_three(batch['inputs_pix']['fy'])
    batch['inputs_pix']['rays_o'] = swap_first_three(batch['inputs_pix']['rays_o'])
    batch['inputs_pix']['rays_d'] = swap_first_three(batch['inputs_pix']['rays_d'])
    batch['inputs_pix']['depth_m'] = swap_first_three(batch['inputs_pix']['depth_m'])
    batch['inputs_pix']['conf_m'] = swap_first_three(batch['inputs_pix']['conf_m'])
    batch['inputs_pix']['sparse_gt_depth'] = swap_first_three(batch['inputs_pix']['sparse_gt_depth'])
    
    ego_pose_for_stereo = batch['inputs_pix']['c2w'][0]
    batch['inputs_pix']['c2w_ego'] = [ego_pose_for_stereo] * len(batch['inputs_pix']['c2w'])
    
    # w2i-ego
    batch['inputs_vol']['w2i'] = swap_first_three(batch['inputs_vol']['w2i'])
    batch['inputs_vol']['w2i_ego'] = [batch['inputs_vol']['w2i'][0]] * len(batch['inputs_vol']['w2i'])
