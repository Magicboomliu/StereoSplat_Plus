...
from .encoder3.backbone import BackboneMultiview
from .encoder3.common.gaussian_adapter import GaussianAdapter, GaussianAdapterCfg
from .encoder3.matching.depth_predictor_trans import DepthPredictorTrans
...
class Transplat(nn.Module):
    ...
    def forward(self,batch, mode="train", iter=0, cfg=None):
        ...
        with torch.no_grad():
            ...
            # 保持高度不变，只调整宽度到patch_size的倍数
            target_h = h  # 保持112不变
            target_w = ((w + patch_size // 2) // patch_size) * patch_size  # 四舍五入到最接近的14倍数
            # 对于w=544: (544 + 7) // 14 * 14 = 551 // 14 * 14 = 39 * 14 = 546
            
            if target_h % patch_size != 0:
                # 如果高度也不是倍数，也调整
                target_h = ((h + patch_size // 2) // patch_size) * patch_size
            
            # 对于112×544: target_h=112, target_w=546 (只改了2像素！)
            da_images = F.interpolate(da_images, (target_h, target_w), mode="bilinear", align_corners=True)
            # ====== 修改结束 ======
            
            da_depth, out_feature = self.da_model.forward(da_images)
            # out_feature: [b*v, C, 112/14, 546/14] = [b*v, C, 8, 39]
            ...
            da_depth = (da_depth - da_min) / (da_max - da_min + 1e-8)
