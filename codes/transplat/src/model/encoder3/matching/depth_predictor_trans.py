...
from .matching_trans import UVTransformer,cam_param_encoder
...
class DepthPredictorTrans(nn.Module):
    ...
    def match_two(self, intr_curr, pose_curr, extrinsics, disp_candi_curr, dino_feature, features):
        ...
        # Resize pos_feature from fixed 64×64 to features' spatial resolution
        if pos_feature.shape[-2:] != (h, w):
            pos_feature = F.interpolate(pos_feature, size=(h, w), mode='bilinear', align_corners=True)
