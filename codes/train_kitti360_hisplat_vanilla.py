...
# Encoder Here
from HiSplat.src.model.hisplat import HiSplatEncoder
# Decoder Here
from HiSplat.src.model.decoder2.my_decoder_splatting_cuda import DecoderSplattingCUDA
...
def main(args):
    ...
    hisplat_encoder = HiSplatEncoder(cfg.unimatch_weights_path, 
                                     cfg.vit_path)
    hisplat_encoder.decoder = hisplat_decoder
    my_model = hisplat_encoder
        ...
        for i_iter, batch in enumerate(train_dataloader):
            ...
            try:
                with accelerator.accumulate(my_model):
                    ...
                    loss, logs,rendered_color,rendered_depth,estimated_gausssains_raw = out
                ...
                if global_iter % cfg.val_freq == 0:
                    ...
                    if accelerator.is_main_process:
                        ...
                        for i_iter_val, batch_val in enumerate(val_dataloader):
                            ...
                            if args.gpus<=1:
                                output_rgb_meter_dict,output_depth_meter_dict = my_model.validation_step(batch_val, val_batch_save_dir,cfg=cfg)
                            else:
                                output_rgb_meter_dict,output_depth_meter_dict = my_model.module.validation_step(batch_val, val_batch_save_dir,cfg=cfg)
                        ...
                        saved_into_json(data_dict=results_dict,
                                        path=os.path.join(val_batch_save_dir,"metric.json"))
