...
class ModelWarpper(nn.Module):
    ...
    def forward(self,batch, mode="train", iter=0, cfg=None):
        ...
        z_near_batch = torch.from_numpy(np.array([cfg.near])).unsqueeze(0).repeat(input_images.shape[0],input_images.shape[1]).type_as(input_images)
        z_far_batch = torch.from_numpy(np.array([cfg.far])).unsqueeze(0).repeat(input_images.shape[0],input_images.shape[1]).type_as(input_images)
