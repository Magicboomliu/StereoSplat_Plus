...
def batch_features_camera_parameters(
    ...
    # query, key and value for transformer
    # 用于存储 每个参考视角的特征图（Query）
    # 每个元素是一个 [B, C, H, W] 的张量，对应某个视角在 batch 中的图像特征
    # 你最终会得到 B × V 个参考视角
    q = [] 
    q_intrinsics = [] # 用于存储每个参考视角的相机内参（[B, 3, 3]）
    q_extrinsics = [] # 用于存储每个参考视角的相机位姿（[B, 4, 4]，通常是 cam2world）
    
    # 用于存储每个参考视角所要与之进行匹配的 邻居视角特征（Key/Value）
    # 每个元素是一个 [B, K-1, C, H, W]，即对该视角而言，K-1 个目标视角的特征图
    kv = []
    kv_intrinsics = [] # 存储目标视角的 intrinsics（[B, K-1, 3, 3]）
    kv_extrinsics = [] # 存储目标视角的 extrinsics（[B, K-1, 4, 4]）

    
    num_views = len(features) # 2
    if nn_matrix is not None:
        ...
        num_selected_views = nn_matrix.size(-1) - 1  # no self-attention here, because it is matching
        
    else:
        num_selected_views = num_views - 1

    
    # 遍历每个视角作为 参考视角（reference view）。
    for i in range(num_views):
        
        # 抽出 Query（自己）
        x = features.copy() 
        q.append(x.pop(i))  # [B, C, H, W] # 把第 i 个视角当作 query，其余的是待选 key-value
        # camera
        y = intrinsics.copy()
        ...
        if nn_matrix is not None:
            ...
            # 用 .gather() 从 features_tensor: [B, V, C, H, W] 中选出 K-1 个目标视角 → [B, K-1, C, H, W]
            kv_x = torch.gather(features_tensor, dim=1, index=index) 
