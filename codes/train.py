...
def main(args):
    ...
    while epoch < max_num_epochs:
        ...
        time_s = time.time()
        for i_iter, batch in tqdm(enumerate(train_dataloader)):
            # forward + backward + optimize
            data_time_e = time.time()
            with accelerator.accumulate(my_model):
                optimizer.zero_grad()
                
                # get the loss and the logs
                
                #FIXME: Change the DDP into SIGNLE GPU Cards
                # loss, log, _, _, _, _, _, _, _ = my_model.module.forward(batch, "train", 
                #                                                          iter=global_iter, 
                #                                                          iter_end=cfg.max_train_steps)

                loss, log, _, _, _, _, _, _, _ = my_model.forward(batch, "train", 
                                                                         iter=global_iter, 
                                                                         iter_end=cfg.max_train_steps)
                
                print("current loss is {}/{}".format(global_iter,loss))
                print("-------------------------------")
            ...
            if accelerator.sync_gradients and accelerator.is_main_process:
                ...
                # saved the model if it is at the  savng prfences
