...
class KITTI360FirstFrameCAM(Dataset):    
    def __init__(
        self,
        datapath:str,
        train_filelist:str,
        ...
        pair_images:int=2,
        world_center='cam0', # Select from 'cam' or 'LiDAR'
        **kwargs,
        ):
        super().__init__()
    ...
    def __getitem__(self, index):
        ...
        # dict_keys(['token', 'scene_token', 'timestep', 
                    #'bin_length', 'original_reference_view_info_dict', 'sensor_info'])
        
        # Original Reference View Info Dict : dict_keys(['left_cam_intrinsic', 'left_cam_to_world', 'left_cam_to_velo', 'velo_to_left_cam', 
                # 'right_cam_intrinsic', 'right_cam_to_world', 'right_cam_to_velo', 'velo_to_right_cam'])
        
        # 
        if self.world_center=='cam0':
        ...
        # default the world source is the LiDAR cooridnate
        elif self.world_center=='LiDAR':
            pass
        ...
        # last
        sensor_info_last = {sensor: bin_info["sensor_info"][sensor][2] for sensor in self.camera_types_first + ["LIDAR_TOP"]}
        ...
        elif self.input_type=='max':
            
            sensor_info_all = sensor_info_all[:self.max_input_views]
        ...
        if self.split=="train":
            ...
            if extra_uniform_selected_supervision_view>0:
                ...
                if self.use_first:
                    rend_indices = [[0, 2,1]+ selected_candidates_view_indices] * len(self.camera_types)
                else:
                ...
                if self.use_first:
                    rend_indices = [[0, 2,1]] * len(self.camera_types)
                else:
                    rend_indices = [[0]] * len(self.camera_types)
        ...
        # pack data
        input_dict = {"rgb": input_imgs_list[1].clone()
                      } # images

        input_dict_pix = {"ck": input_cks_list[1].clone(), "c2w": input_c2ws_list[1].clone(),
                          "cx": input_cxs_list[1].clone(), "cy": input_cys[1].clone(), "fx": input_fxs_list[1].clone(), "fy": input_fys_list[1].clone(),
                          "rays_o": input_rays_o_list[1].clone(), "rays_d": input_rays_d_list[1].clone()}

        
        if self.depth_info_dict.use_pseudo_depth:
            input_dict_pix.update({
                                    "depth_m":input_depths_m_list[1].clone(),
                                    "conf_m":input_confs_m_list[1].clone()
                                    })
            
        if self.depth_info_dict.use_sparse_lidar:
            input_dict_pix.update({
                                    "sparse_gt_depth":input_sparse_gt_depth_list[1].clone()
                                    })
        
        input_dict_vol = {"w2i": input_w2is_list[1].clone()} # volume based methods
        ...
        output_dict = {"rgb": output_imgs[re_ordered_index], 
                    ...
                    # "input_image_path":output_img_paths+ input_img_paths[1]
                    }
...
if __name__=="__main__":
    ...
    def save_dict_to_pickle(data, filepath):
        """
        Save a dictionary or list of dictionaries to a .pkl file.

        Args:
            data (dict or list of dict): The dictionary (or list of dicts) to save.
            filepath (str): The path to the output .pkl file.
        """
    ...
    val_dataset = KITTI360FirstFrameCAM(**dataset_params)
