...
# def self.swap_first_three(lst, A, B):
...
class KITTI360DatasetComplete(Dataset):    
    ...
    def __getitem__(self, index):
        ...
        # pack data
        input_dict = {"rgb": input_imgs_list[1],
                      "rgb_path": input_rgb_names_list[1],
                      } # images

        input_dict_pix = {"ck": input_cks_list[1], "c2w": input_c2ws_list[1],
                          "cx": input_cxs_list[1], "cy": input_cys[1], 
                          "fx": input_fxs_list[1], "fy": input_fys_list[1],
                          "rays_o": input_rays_o_list[1], "rays_d": input_rays_d_list[1]}

        
        if self.depth_info_dict.use_pseudo_depth:
            input_dict_pix.update({
                                    "depth_m":input_depths_m_list[1],
                                    "conf_m":input_confs_m_list[1]
                                    })
            
        if self.depth_info_dict.use_sparse_lidar:
            input_dict_pix.update({
                                    "sparse_gt_depth":input_sparse_gt_depth_list[1]
                                    })
        
        input_dict_vol = {"w2i": input_w2is_list[1]} # volume based methods
