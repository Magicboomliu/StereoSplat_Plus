...
# define the nuscecne dataset
class nuScenesDataset(Dataset):
    
    #
    data_root: str = "/data2/zliu_backup_data/FeedForwardGS_Datasest/nuscence/subset"
    
    data_version: str = "interp_12Hz_trainval" # saved the json files
    ...
    def __init__(
        ...
        # training the inference resolution？
        ...
        # load bin tokens, tokens is 
        if split == "train":
            #for training
            # FIXME Here, just to see the training propocess
    ...
    def __getitem__(self, index):
        ...
        bin_token = self.bin_tokens[index] # get the bin token
        
    
        with open(osp.join(self.data_root, self.data_version, "bin_infos_3.2m", bin_token + ".pkl"), "rb") as f:
            bin_info = pkl.load(f) # bin information is what?
        
        # dict_keys(['LIDAR_TOP', 'CAM_FRONT', 'CAM_FRONT_RIGHT', 
        ...
        if self.use_last:
            for cam in self.camera_types_last:
                ...
                input_c2ws.append(c2w)
                input_w2cs.append(w2c)
        input_c2ws = torch.as_tensor(input_c2ws, dtype=torch.float32) #(6,4,4)
        input_w2cs = torch.as_tensor(input_w2cs, dtype=torch.float32) #(6,4,4)
        ...
        input_w2is = torch.stack(input_w2is) #(6,4,4), here is All Center
        ...
        # here 1,2 means the top and the bootom
        if self.use_center:
            rend_indices = [[1, 2]] * 6 # [[1, 2], [1, 2], [1, 2], [1, 2], [1, 2], [1, 2]]--> Frist and the Last
        else:
            rend_indices = [[0]] * 6
        ...
        output_c2ws = torch.as_tensor(output_c2ws, dtype=torch.float32)  #(12,4,4)-->(2*6)
