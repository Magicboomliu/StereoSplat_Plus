...
def get_ray_directions(
    ...
    """
    # 计算每个像素在相机坐标系下的视线方向（单位为相机坐标，不带旋转）
    ...
    """
    pixel_center = 0.5 if use_pixel_centers else 0 # using the pixel center, which is more realible
    ...
    ) # make sure Z is forward, y is upper
...
def get_rays(
    ...
    if directions.ndim == 2:  # (N_rays, 3)
        ...
        rays_d = (directions[:, :, :, None, :] * c2w[:, None, None, :3, :3]).sum(
            -1
        )  # (B, H, W, 3) # 批量化的“方向向量 × 相机旋转矩阵”的矩阵乘法操作
        rays_o = c2w[:, None, None, :3, 3].expand(rays_d.shape)
...
def get_cam_info_gaussian(c2w, fovx, fovy, znear, zfar):
    '''
    - world_view_transform:世界到相机
    - full_proj_transform: 加上投影后的变换
    ...
    '''
    1. 转置矩阵（行列互换）
    2. 放到 GPU 上，并确保是 float 类型。
        一般是因为 rasterizer期望输入是列优先, OpenGL-style
    '''
    world_view_transform = world_view_transform.transpose(0, 1).cuda().float()
    '''
    生成一个基于 fovx、fovy、znear、zfar 的投影矩阵 Perspective Projection
    再转置、放到 GPU。
    get_projection_matrix_gaussian()应该内部是建一个4x4的标准透视投影矩阵。
    类似 OpenGL 的 glPerspective
    '''
    projection_matrix = (
    ...
    # 表示从世界空间 ➔ 相机空间 ➔ 屏幕空间的一步变换！
    full_proj_transform = (
        world_view_transform.unsqueeze(0).bmm(projection_matrix.unsqueeze(0))
    ).squeeze(0).float()
    
    # cam center is c2w 的第四行前三列就是 camera的位置坐标。
