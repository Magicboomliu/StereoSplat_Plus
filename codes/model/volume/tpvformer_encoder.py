...
class TPVFormerEncoder(TransformerLayerSequence):
    ...
    def __init__(self,
        ...
        # 投影特征的transform卷积（非TPV-only模式）, have feature from pixel gaussains
        if not tpv_only:
    ...
    def forward(self, mlvl_feats, project_feats, img_metas):
        ...
        # [4,6,128,56,100]
        # # 图像特征维度：(B, N_cam, C, H, W)
        bs = mlvl_feats[0].shape[0] # batch size
        ...
        tpv_queries_hw = tpv_queries_hw.unsqueeze(0).repeat(bs, 1, 1) #((4,192*192,128)
        tpv_queries_zh = tpv_queries_zh.unsqueeze(0).repeat(bs, 1, 1) #(4,192*16,128)
        tpv_queries_wz = tpv_queries_wz.unsqueeze(0).repeat(bs, 1, 1) #(4,16*192,128)
        

        # add projected feats to tpv queries
        # # 将三视图投影特征投到TPV query上, here means the feature from the pxiel gs
        if project_feats[0] is not None and project_feats[1] is not None and project_feats[2] is not None:
            ...
            tpv_queries_hw = tpv_queries_hw + project_feats_hw # update here
        ...
        # # 多层图像特征压平成统一格式 (num_cam, H*W, bs, C)
        feat_flatten = []
        spatial_shapes = []
