...
class VolumeFusionRevision_Volume3D_Branch_Only(BaseModule):
    ...
    def __init__(self,
        ...
        # Cost Volume 分支已移除，只保留 Volume Gaussian 分支
        # self.costvolume_gs = CostVolumeGS(**costvolume_gs)  # 已移除
    ...
    def generate_pointcloud_from_depth(self, depth, intrinsics, extrinsics, img_feats):
        """
        从深度图生成点云和特征（用于替代 Cost Volume 分支）
        
        Args:
            depth: [B, V, H, W] 输入深度图
            intrinsics: [B, V, 3, 3] 相机内参
            extrinsics: [B, V, 4, 4] 相机外参 (c2w)
            img_feats: [B, V, C, H_feat, W_feat] 图像特征
        
        Returns:
            pointcloud: [B, N, 14] 点云，前3维是真实3D位置，其余是占位符
            features: [B, N, C] 对应的特征
        """
        B, V, H, W = depth.shape
        _, _, C, H_feat, W_feat = img_feats.shape
        
        # 1. 使用现有函数生成3D点云
        pointmap = get_pointmap_from_depth(depth, intrinsics, extrinsics)  # [B, V, H, W, 3]
        
        # 2. 从特征图采样特征（调整到与深度图相同分辨率）
        feats = F.interpolate(img_feats.reshape(B*V, C, H_feat, W_feat), 
                              size=(H, W), mode='bilinear', align_corners=False)
        feats = feats.reshape(B, V, C, H, W)
        
        # 3. Flatten 成点的形式
        points_xyz = pointmap.reshape(B, V*H*W, 3)  # [B, N, 3]
        points_feats = feats.permute(0, 1, 3, 4, 2).reshape(B, V*H*W, C)  # [B, N, C]
        
        # 4. 填充其他维度（RGB, opacity, rotation, scale）保持格式兼容
        # 14维 = 3(xyz) + 3(rgb) + 1(opacity) + 4(rotation) + 3(scale)
        rgb = torch.ones(B, V*H*W, 3, device=depth.device, dtype=depth.dtype) * 0.5
        opacity = torch.zeros(B, V*H*W, 1, device=depth.device, dtype=depth.dtype)
        rotation = torch.tensor([1.0, 0.0, 0.0, 0.0], device=depth.device, dtype=depth.dtype).reshape(1, 1, 4).expand(B, V*H*W, 4)
        scale = torch.ones(B, V*H*W, 3, device=depth.device, dtype=depth.dtype) * 0.1
        
        pointcloud = torch.cat([points_xyz, rgb, opacity, rotation, scale], dim=-1)  # [B, N, 14]
        
        return pointcloud, points_feats
    ...
    def forward(self,batch,
        ...
        render_pkg_volume = render_pkg_fuse
        ...
        if mode=='train' or mode=='val':
            ...
            # 注意：由于移除了 Cost Volume 深度估计分支，pred_depths 直接使用输入深度
            # 因此深度估计损失被跳过
            depth_estimation_branch_loss = torch.tensor(0.0, device=self.device)
            ...
            loss = trip_plane_branch_loss * self.losses_params.volume_sup_dict.branch_weight + \
