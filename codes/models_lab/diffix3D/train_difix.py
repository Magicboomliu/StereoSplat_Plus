...
def _pad_to_min_hw(x: torch.Tensor, min_h: int, min_w: int, mode: str = "reflect"):
    ...
    if pad_h == 0 and pad_w == 0:
        return x
    # F.pad takes pads as (left, right, top, bottom) for 4 last dims; works for arbitrary leading dims
    return F.pad(x, (pad_w // 2, pad_w - pad_w // 2,
                     pad_h // 2, pad_h - pad_h // 2), mode=mode)

def safe_random_crop(x: torch.Tensor, crop_h: int, crop_w: int, *, pad_if_needed=True, pad_mode="reflect"):
    """
    ...
    if pad_if_needed:
        ...
        H, W = x.shape[-2], x.shape[-1]
        ch = min(crop_h, H); cw = min(crop_w, W)
...
def main(args):
    ...
    # for the evaluations & loss computations
    ...
    # VAE Lora Partical Optimization
