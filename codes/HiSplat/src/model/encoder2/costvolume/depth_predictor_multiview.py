...
class DepthPredictorMultiViewPyramid(nn.Module):
    ...
    def forward(
        ...
        for i in range(len(self.depth_predictor)):
            ...
            if i != 0:
                ...
                pre_stage_context_image,pre_stage_context_depth = encoder.decoder.forward(
                    pre_stage_gaussian,
                    extrinsics,
                    intrinsics,
                    near,
                ...
                pre_stage_context_image = pre_stage_context_image # b v c h w
    ...
    def extract_raw_guassians(
        ...
        # Handle non-square images by finding factors
        import math
        factors = []
        for i in range(1, int(math.sqrt(r)) + 1):
            if r % i == 0:
                factors.append((i, r // i))
        ...
        for j in range(stage_id):
            ...
            # Handle non-square images for previous stage too
            factors_pre = []
            for i in range(1, int(math.sqrt(r_pre)) + 1):
                if r_pre % i == 0:
                    factors_pre.append((i, r_pre // i))
            h_pre, w_pre = min(factors_pre, key=lambda x: abs(x[0] - x[1]))
